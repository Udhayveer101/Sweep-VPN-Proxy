...
# commit plus masque-keepalive.patch (HTTP/2 PINGs + in-tunnel DNS retry; see
# WarpController.arguments for why). Universal so the release runs on Intel too.
#
...
git -C "$WORK" -c user.name=ci -c user.email=ci@localhost am -q "$HERE/masque-keepalive.patch"
